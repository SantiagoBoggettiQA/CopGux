...
cy.wait(2000)
cy.get('.col-3 > .component-menu > :nth-child(3) > .m-0 > [href="/pedidos"] > .color-grey-menu').click({force: true});
cy.get('.position-absolute > :nth-child(1)').click();
cy.wait(2000)
cy.get(':nth-child(3) > .rs-copec-cleanable > .rs-picker-toggle > .rs-picker-toggle-value').click();
cy.wait(1500)
cy.get('[data-key="NEW"] > .rs-picker-select-menu-item').click();
cy.wait(1000)
cy.get('.position-absolute > [value=""]').click();
